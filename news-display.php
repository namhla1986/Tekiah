<?php require("inc-public-predoctype.php"); ?>
<?php 
$vid = $_GET['kid'];

//CREATE SQL STATEMENT
$sql_n = "SELECT * FROM tblnews WHERE nid = $vid";

//CONNECT TO THE MYSQL SERVER 
require('inc-conntekiah.php');

//EXECUTE SQL STATEMENT
$rs_n = mysqli_query($vconntekiah, $sql_n);

//CREATE AN ASSOCIATIVE ARRAY
$rs_news_rows = mysqli_fetch_assoc($rs_n);
?>
<!DOCTYPE HTML>
<html>

<head>
<?php require("inc-head-content.php"); ?>

</head>

<body>
<!-- WRAPPER -->
<section id="wrapper">

<!-- HEADER -->
<?php require('inc-header-content.php'); ?>

<script type="application/ld+json">
{
  "@context": "http://schema.org",
  "@type": "NewsArticle",
  "headline": <?php echo json_encode($rs_news_rows['ntitle'], 64 | 128 | 256); ?>,
  "Image": {
	  "@type": "ImageObject",
	  "thumbnail": 
	  	[
		<?php echo json_encode(
		"https://namhla-mankune.webportfolio.capetown/tekiah/uploaded-images/" . $rs_news_rows['nimgthumb']); ?>, 
		<?php echo json_encode(
		"https://namhla-mankune.webportfolio.capetown/tekiah/uploaded-images/" . $rs_news_rows['nimglarge']); ?> 
		],
		
	"caption": <?php echo json_encode($rs_news_rows['nimglargecaption']); ?>
  },
  "datePublished": <?php echo json_encode($rs_news_rows['nmodified'], 64 | 128 | 256); ?>,
  "description": <?php echo json_encode($rs_news_rows['ncontent'], 64 | 128 | 256); ?>,
  "description": <?php echo json_encode($rs_news_rows['nsummary'], 64 | 128 | 256); ?>,
}
</script>
<!-- Appear only on tablet layout and higher. Replaces mobile nav bar -->
<!--Tablet and higher nav bar -->
<?php require('inc-navbar-desktop.php'); ?>

<!-- CONTENT -->
<section id="content_container">

<!-- MOBILE HEADER WITH NAV ICON -->
<?php require('inc-navbar-mobile.php'); ?>
<!-- NAV -->

<?php require('inc-navbar-mobile.php'); ?>

<!-- Appear on all layouts -->
<section id="content_left">

<!-- Appear on Desktop only -->
<article id="content_left_article_1">
<!-- SIGN IN HEADING AND FORM-->
		<h1><strong>News</strong></h1>  
        
        <article class="newspiece">
        
                <figure class="largeimg">
				<img src="cms/uploaded-images/<?php echo $rs_news_rows['nimglarge'] ?>"> 
				<figcaption><?php echo $rs_news_rows['nimglargecaption'] ?><figcaption>
                </figure>
                
                <div class="newsblock">
                <h2><?php echo $rs_news_rows['ntitle']; ?></h2>
                <p><?php echo $rs_news_rows['ndatetime']; ?></p>
                
                <br>
                
                <div class="content">
                <p><?php echo $rs_news_rows['ncontent']; ?></p>
        		</div>
                
				<div class="clear_float"></div>
                
                <hr>
                
        </article>
	</article>


</section>

<!-- Appear only on tablet layout and higher -->
<section id="content_right">

    <!-- CONTENT RIGHT -->
    <?php require('inc-right-column.php'); ?>
</section></section>

<footer id="my_footer">

<!-- FOOTER CONTENT -->
<?php require('inc-footer-content.php'); ?>

</footer>

<br class="clear_float" />

</section>

</body>
</html>
